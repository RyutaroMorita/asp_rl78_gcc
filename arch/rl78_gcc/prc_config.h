 ...
 */
Inline void
set_isp(uint8_t iipm)
{
	set_psw((current_psw() & ~0x06U) | iipm);
}
...
extern volatile uint8_t		saved_iipm;	/* 割込み優先度マスクを保存する変数 */
...
x_lock_cpu(void)
{
	uint8_t	isp;
	/*
	 *  current_isp()の返り値を直接saved_iipmに保存せず，一時変数isp
	 *  を用いているのは，current_isp()を呼んだ直後に割込みが発生し，
 ...
 *
 *  TMIN_INTPRIの値によらず，chg_ipmでは，-3～TIPM_ENAALL（＝0）の範囲
 *  に設定できることとする（ターゲット定義の拡張）．
 */
#define VALID_INTPRI_CHGIPM(intpri) \
				(-3 <= (intpri) && (intpri) <= TIPM_ENAALL)
